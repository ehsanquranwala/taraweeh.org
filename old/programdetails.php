...
if(isset($_GET['prid']))$PrgId = $_GET['prid']; else $PrgId = 1;

mysqli_select_db($connDoraTarjumaQuran, $database_connDoraTarjumaQuran);
$query_rs_Views = "SELECT u.UserId, CONCAT_ws(' ',Prefix, FirstName, LastName) FullName, pf.ViewHeading, pf.CompleteExperience FROM user u, programfeedback pf WHERE u.UserId=pf.UserId and pf.ProgramId=".$PrgId;
$rs_Views = mysqli_query($connDoraTarjumaQuran, $query_rs_Views) or die(mysqli_error());
$row_rs_Views = mysqli_fetch_assoc($rs_Views);

mysqli_select_db($connDoraTarjumaQuran, $database_connDoraTarjumaQuran);
$query_rs_ProgramDetails = "SELECT pl.PlaceId, pr.ProgramId, pr.Year, pr.HijriYear, pr.StartTime, pr.EndTime, pr.ProgramDetails, CONCAT_WS(', ',pt.Mode, pt.Translation, pt.Time) ProgramType, pt.Gender FROM place pl join program pr join programtype pt WHERE pr.ProgramId = ".$PrgId." and pr.PlaceId = pl.PlaceId and pr.ProgramTypeId = pt.ProgramTypeId";
...
$programdetails = '';
$prg=$PrgId;
$row_rs_ProgramDetails = mysqli_fetch_assoc($rs_ProgramDetails);
$PlcId=$row_rs_ProgramDetails['PlaceId'];
...
<tr><td><h1 align="center">Dora Tarjuma Quran Program Details - <?php echo $row_rs_ProgramDetails['HijriYear'].' ('.$row_rs_ProgramDetails['Year'].')';?></h1>
...
<tr><td align="center"bgcolor="#99CC66"><h2 align="center">Views about this program</h2></td></tr>
 <?php do { ?>
    <tr>
...
<?php echo $row_rs_Views['CompleteExperience']; ?><br  /></td></tr>
    <?php } while ($row_rs_Views = mysqli_fetch_assoc($rs_Views)); ?>
<tr><td></td></tr>
<tr><td align="center"bgcolor="#99CC66"><h2 align="center">History of this place</h2>
<table width="800" border="1" align="center" cellpadding="0" bgcolor="#CCEEAA"  cellspacing="0">
  <tr bgcolor="#99CC66">
  ...
  <?php
   do { ?>
    <tr>
	  ...
	  if($Admin)echo '<a href="editprogram.php'; else echo '<a href="programdetails.php';echo '?prid='.$row_rs_PlaceHistory['ProgramId'].'">'.$prgyr.'</a>';  }?></td>
